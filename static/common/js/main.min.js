...
function CarouselObj(){this.container;
this.gallery;this.page;this.slides;}
function FormObj(){this.rangeTickWidth=2;this.progressDottedDotWidth=10;}
function MapObj(){this.map;this.view;this.defaultIcon="http://maps.google.com/mapfiles/ms/icons/red-dot.png";
...
function ReportObj(){this.svg;this.legendBoxSize=30;this.legendBoxSpacing=10;this.graphType;this.width;this.height;this.jsonURL;
...
});function init(){modalObj.init();mainMenuObj.init();listViewObj.init();formObj.init();taskListObj.init();calendarObj.init();initializePage();}ModalObj.prototype.init=function(){$(".open-modal").click(function(a){a.preventDefault();
...
{var a=this;a.setupCollapsableMenus();a.setupTabbedLists();a.setupMilestoneLists();a.setupSwipeLis();
	$(window).on("resize",function(){a.setupTabbedLists();});
$(window).on("orientationchange",
	function(){a.setupTabbedLists();});};
ListViewObj.prototype.setupCollapsableMenus=function()
{$.each($("ul.list-view.collapsable > li"),
	function(){$("> .content",this).click(function(b){b.preventDefault();
var a=$(".list-view-icons span",this).last();if($(this).hasClass("expanded")){$(this)
	.removeClass("expanded").siblings("ul").first().slideUp();
...
{var b=this,a=$("ul.tabbed-list-view");
panels=a.children("li"),tabs=$(".tabbed-list-view-nav a"),
tabWidth=$(window).width()/5;
//tabs.first().width(),
arrow=$(".tabbed-list-view-nav-arrow"),
ww=panels.first().width();
arrow.css({left:tabWidth/2});
console.log("Tabwidth: " + tabWidth);
console.log("panels: "+ panels.width());

$.each(panels,function(){$(this).css({left:(panels.index(this)*ww)+"px"});});
tabs.click(function(c)
	{c.preventDefault();$(tabs).removeClass("on");
$(this).addClass("on");a.attr("class","tabbed-list-view").addClass("slide-"+tabs.index(this));

arrow.animate({left:(tabWidth/2)+((tabWidth)*tabs.index(this))},
	{queue:false,duration:200});
console.log("tab1 "+ ((tabWidth/2)+((tabWidth)*tabs.index(this))));
});
};


ListViewObj.prototype.setupMilestoneLists=function(){
	var b=this,a=$(".list-view-milestones");
	$.each(a,function(){
		var d=$("li",this),c="<div class='background-fill'></div>";
		d.first().prepend(c);d.last().prepend(c);
	});};
ListViewObj.prototype.setupSwipeLis=function(){
	var b=this,a=$("ul.with-swipe li");
	if(a.length>0){
		$("ul.with-swipe li").touchwipe(
		{	wipeRight:function(c){
				$(this.el).addClass("swiped");},
			wipeLeft:function(c){$(this.el).removeClass("swiped");},
			min_move_y:20,preventDefaultEvents:true
		});
	}
};
...
TaskListObj.prototype.setupCheckboxes=function(){
		var a=this,b=$(".list-view-tasks li");
		$.each(b,function(){
			var c=this,d=$(".content > .task-complete-checkbox > input",this);
			$(".content",this).prepend("<div class='strikethrough'>&nbsp;</div>").prepend("<div class='overdue-color-block'>&nbsp;</div>");
			d.click(function(){
				$(c).toggleClass("complete");
				$("p",c).slideToggle(200);
			});
		});
};
CalendarObj.prototype.init=function(){
	var b=this,a=$("ul.week-planner li");
	a.click(function(){
		a.removeClass("on");
		$(this).addClass("on");
		$(".date-content").hide();
...
CarouselObj.prototype.init=function(a){var c=this,b,d;
...
a.renderProgressDotted();a.renderRangeTicks();});};FormObj.prototype.checkDateInputs=function()
{var a=document.createElement("input"),b="date";a.setAttribute("type",b);
if(a.type!=b){$.each($(".form-control-date input"),function(){$(this).Zebra_DatePicker();});}};
FormObj.prototype.setupToggles=function(){$.each($(".form-control-toggle"),
	function(){var a=$("input",this).first();
$(this).prepend('<span class="label on"></span><span class="label off"></span><span class="handle">&nbsp;</span>');$(".on",this).html($(this).data("on-label"));
$(".off",this).html($(this).data("off-label"));if(a.prop("checked")==true)
{$(this).addClass("checked");}$(this).click(function(b){b.preventDefault();$(this).toggleClass("checked");
if(a.prop("checked")==true){a.prop("checked",false);}else{a.prop("checked",true);}});});};
FormObj.prototype.renderRangeTicks=function(){var a=this;$.each($(".form-control-range"),
	function(){var l=1,m=l,f=$(this).width()-(2*l),n=$(this).data("tick-labels").split(","),h=Math.round((f-n.length*a.rangeTickWidth)/(n.length-1)),d=f-(h*(n.length-1)+a.rangeTickWidth*n.length),b=$(".noUiSlider",this).val();
...
if(!isNaN(b)){c.val(b);}else{c.val(0);}for(var e=0;e<n.length;e++){var g="",k=0;if(d<0){k=-1;d++;}if(d>0){k=1;d--;}
g=" style='left:"+(m)+"px;'";output+="<li"+g+"><span>"+n[e]+"</span></li>";
...
{var b=this,a='<div class="progress-bar-fill">&nbsp;</div>';$.each($(".progress-bar"),
	function(){$(this).html(a);
$(".progress-bar-fill",this).css({width:$(this).data("completed")+"%"});});};
FormObj.prototype.renderProgressDotted=function()
{var a=this;$.each($(".progress-dotted"),function()
	{var o=$(this).data("completed").split("/"),n=0,f=$(this).width(),g=Math.round((f-o[1]*a.progressDottedDotWidth)/(o[1]-1)),d=f-(g*(o[1]-1)+a.progressDottedDotWidth*o[1]),b=$(".progress-dotted-label",this),l=a.progressDottedDotWidth*(o[0]-0.5)+g*(o[0]-1)-parseInt(b.width())/2,c="<div class='progress-dotted-bg'>&nbsp;</div><ul>";
for(var e=0;e<o[1];e++)
	{var m=(e==o[0]-1?" class='on'":""),h="",k=0;if(e!=0){if(d<0){k=-1;d++;}if(d>0){k=1;d--;}n+=(g+k+a.progressDottedDotWidth);
h=" style='left:"+(n)+"px;'";
...
if(a[c].contentString)
	{var e=new google.maps.InfoWindow;d.bindInfoWindowToMarker(b,a[c].contentString,e);}d.markers.push({id:a[c].id,marker:b,distanceToUser:g});
...
var f=d3.scale.category20();var e=d3.layout.pie().sort(null);var h=d3.svg.arc().innerRadius(a-100).outerRadius(a-50);
var m=c.svg.selectAll("path").data(e(k)).enter().append("path").attr("fill",function(o,n){return f(n);
...
function initializePage(){
	...
	$(".version_a_sessions").click(woopra);
